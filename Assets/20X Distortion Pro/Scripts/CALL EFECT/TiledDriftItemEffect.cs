// FILE: TiledDriftItemEffect.cs
// Drives TiledDriftDistortion.Amount 0 -> target -> 0.
...
public class TiledDriftItemEffect : MonoBehaviour, IItemEffect
{
    ...
    private void Awake()
    {
        if (profile == null)
        {
            ...
            if (global != null)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[TiledDriftItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[TiledDriftItemEffect] No VolumeProfile.");
        }
    }
    ...
    private IEnumerator RunEffect(float duration, float intensity)
    {
        if (profile == null)
        {
            if (verboseLogs) Debug.LogWarning("[TiledDriftItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<TiledDriftDistortion>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[TiledDriftItemEffect] No TiledDriftDistortion.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[TiledDriftItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Amount.value = 0f;
        if (verboseLogs) Debug.Log("[TiledDriftItemEffect] Done, reset.");
    ...
    private void ResetEffect()
    {
        ...
        if (profile != null && profile.TryGet<TiledDriftDistortion>(out var fx) && fx != null)
        {
            fx.Amount.overrideState = true;
            fx.Amount.value = 0f;
        }
