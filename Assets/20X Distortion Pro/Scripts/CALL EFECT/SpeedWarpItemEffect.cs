// FILE: SpeedWarpItemEffect.cs
// Drives SpeedWarp.Intensity 0 -> target -> 0.
...
public class SpeedWarpItemEffect : MonoBehaviour, IItemEffect
{
    ...
    private void Awake()
    {
        if (profile == null)
        {
            ...
            if (global != null)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[SpeedWarpItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[SpeedWarpItemEffect] No VolumeProfile.");
        }
    }
    ...
    private IEnumerator RunEffect(float duration, float intensity)
    {
        if (profile == null)
        {
            if (verboseLogs) Debug.LogWarning("[SpeedWarpItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<SpeedWarp>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[SpeedWarpItemEffect] No SpeedWarp.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[SpeedWarpItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Intensity.value = 0f;
        if (verboseLogs) Debug.Log("[SpeedWarpItemEffect] Done, reset.");
    ...
    private void ResetEffect()
    {
        ...
        if (profile != null && profile.TryGet<SpeedWarp>(out var fx) && fx != null)
        {
            fx.Intensity.overrideState = true;
            fx.Intensity.value = 0f;
        }
