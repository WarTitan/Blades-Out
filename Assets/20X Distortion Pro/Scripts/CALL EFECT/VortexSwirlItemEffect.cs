// FILE: VortexSwirlItemEffect.cs
// Drives VortexSwirlDisplacement.Fade 0 -> target -> 0.
...
public class VortexSwirlItemEffect : MonoBehaviour, IItemEffect
{
    ...
    private void Awake()
    {
        if (profile == null)
        {
            ...
            if (global != null)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[VortexSwirlItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[VortexSwirlItemEffect] No VolumeProfile.");
        }
    }
    ...
    private IEnumerator RunEffect(float duration, float intensity)
    {
        if (profile == null)
        {
            if (verboseLogs) Debug.LogWarning("[VortexSwirlItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<VortexSwirlDisplacement>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[VortexSwirlItemEffect] No VortexSwirlDisplacement.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[VortexSwirlItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Fade.value = 0f;
        if (verboseLogs) Debug.Log("[VortexSwirlItemEffect] Done, reset.");
    ...
    private void ResetEffect()
    {
        ...
        if (profile != null && profile.TryGet<VortexSwirlDisplacement>(out var fx) && fx != null)
        {
            fx.Fade.overrideState = true;
            fx.Fade.value = 0f;
        }
