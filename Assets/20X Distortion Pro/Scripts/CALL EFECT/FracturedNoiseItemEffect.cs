...
public class FracturedNoiseItemEffect : MonoBehaviour, IItemEffect
{
    ...
    void Awake()
    {
        if (!profile)
        {
            ...
            if (global)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[FracturedNoiseItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[FracturedNoiseItemEffect] No VolumeProfile.");
        }
    }
    ...
    IEnumerator RunEffect(float duration, float intensity)
    {
        if (!profile)
        {
            if (verboseLogs) Debug.LogWarning("[FracturedNoiseItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<FracturedNoiseDisplacement>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[FracturedNoiseItemEffect] No FracturedNoiseDisplacement.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[FracturedNoiseItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Amount.value = 0f;
        if (verboseLogs) Debug.Log("[FracturedNoiseItemEffect] Done, reset.");
    ...
    void ResetEffect()
    {
        ...
        if (profile && profile.TryGet<FracturedNoiseDisplacement>(out var fx) && fx != null)
        {
            fx.Amount.overrideState = true;
            fx.Amount.value = 0f;
        }
