// FILE: RecursiveVortexWarpItemEffect.cs
// Drives RecursiveVortexWarp.Fade 0 -> target -> 0.
...
public class RecursiveVortexWarpItemEffect : MonoBehaviour, IItemEffect
{
    ...
    void Awake()
    {
        if (!profile)
        {
            ...
            if (global)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[RecursiveVortexWarpItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[RecursiveVortexWarpItemEffect] No VolumeProfile.");
        }
    }
    ...
    IEnumerator RunEffect(float duration, float intensity)
    {
        if (!profile)
        {
            if (verboseLogs) Debug.LogWarning("[RecursiveVortexWarpItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<RecursiveVortexWarp>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[RecursiveVortexWarpItemEffect] No RecursiveVortexWarp.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[RecursiveVortexWarpItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Fade.value = 0f;
        if (verboseLogs) Debug.Log("[RecursiveVortexWarpItemEffect] Done, reset.");
    ...
    void ResetEffect()
    {
        ...
        if (profile && profile.TryGet<RecursiveVortexWarp>(out var fx) && fx != null)
        {
            fx.Fade.overrideState = true;
            fx.Fade.value = 0f;
        }
