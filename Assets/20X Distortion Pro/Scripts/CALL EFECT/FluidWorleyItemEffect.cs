// FILE: FluidWorleyItemEffect.cs
// Drives FluidWorleyDisplacement.Intensity 0 -> target -> 0.
...
public class FluidWorleyItemEffect : MonoBehaviour, IItemEffect
{
    ...
    void Awake()
    {
        if (profile == null)
        {
            ...
            if (global)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[FluidWorleyItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[FluidWorleyItemEffect] No VolumeProfile.");
        }
    }
    ...
    IEnumerator RunEffect(float duration, float intensity)
    {
        if (!profile)
        {
            if (verboseLogs) Debug.LogWarning("[FluidWorleyItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<FluidWorleyDisplacement>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[FluidWorleyItemEffect] No FluidWorleyDisplacement.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[FluidWorleyItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Intensity.value = 0f;
        if (verboseLogs) Debug.Log("[FluidWorleyItemEffect] Done, reset.");
    ...
    void ResetEffect()
    {
        ...
        if (profile && profile.TryGet<FluidWorleyDisplacement>(out var fx) && fx != null)
        {
            fx.Intensity.overrideState = true;
            fx.Intensity.value = 0f;
        }
