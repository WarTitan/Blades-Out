// FILE: SectorKaleidoItemEffect.cs
// Drives SectorKaleidoDistort.Fade 0 -> target -> 0.
...
public class SectorKaleidoItemEffect : MonoBehaviour, IItemEffect
{
    ...
    void Awake()
    {
        if (!profile)
        {
            ...
            if (global)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[SectorKaleidoItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[SectorKaleidoItemEffect] No VolumeProfile.");
        }
    }
    ...
    IEnumerator RunEffect(float duration, float intensity)
    {
        if (!profile)
        {
            if (verboseLogs) Debug.LogWarning("[SectorKaleidoItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<SectorKaleidoDistort>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[SectorKaleidoItemEffect] No SectorKaleidoDistort.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[SectorKaleidoItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Fade.value = 0f;
        if (verboseLogs) Debug.Log("[SectorKaleidoItemEffect] Done, reset.");
    ...
    void ResetEffect()
    {
        ...
        if (profile && profile.TryGet<SectorKaleidoDistort>(out var fx) && fx != null)
        {
            fx.Fade.overrideState = true;
            fx.Fade.value = 0f;
        }
