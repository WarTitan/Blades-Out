// FILE: ZigzagJitterItemEffect.cs
// Attach this to a prefab root (e.g. ZigzagJitterEffectPrefabRoot).
// Drives ZigzagJitterDisplacement.Intensity 0 -> target -> 0.
...
public class ZigzagJitterItemEffect : MonoBehaviour, IItemEffect
{
        ...
        if (profile == null)
        {
            ...
            if (volumes != null)
                for (int i = 0; i < volumes.Length; i++)
                    if (volumes[i] != null && volumes[i].isGlobal) { global = volumes[i]; break; }

            if (global != null)
            {
                profile = global.profile;
                if (verboseLogs)
                    Debug.Log("[ZigzagJitterItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[ZigzagJitterItemEffect] No VolumeProfile.");
        }
    }
    ...
    private IEnumerator RunEffect(float duration, float intensity)
    {
        if (profile == null)
        {
            if (verboseLogs) Debug.LogWarning("[ZigzagJitterItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<ZigzagJitterDisplacement>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[ZigzagJitterItemEffect] No ZigzagJitterDisplacement in profile.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[ZigzagJitterItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Intensity.value = 0f;
        if (verboseLogs) Debug.Log("[ZigzagJitterItemEffect] Done, reset.");
    ...
    private void ResetEffect()
    {
        ...
        if (profile != null && profile.TryGet<ZigzagJitterDisplacement>(out var fx) && fx != null)
        {
            fx.Intensity.overrideState = true;
            fx.Intensity.value = 0f;
        }
