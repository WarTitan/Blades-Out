// FILE: SpiralSliceKaleidoscopeItemEffect.cs
// Drives SpiralSliceKaleidoscope.Fade 0 -> target -> 0.
...
public class SpiralSliceKaleidoscopeItemEffect : MonoBehaviour, IItemEffect
{
    ...
    private void Awake()
    {
        if (profile == null)
        {
            ...
            if (global != null)
            {
                ...
                if (verboseLogs)
                    Debug.Log("[SpiralSliceKaleidoscopeItemEffect] Auto profile from " + global.name);
            }
            else if (verboseLogs)
                Debug.LogWarning("[SpiralSliceKaleidoscopeItemEffect] No VolumeProfile.");
        }
    }
    ...
    private IEnumerator RunEffect(float duration, float intensity)
    {
        if (profile == null)
        {
            if (verboseLogs) Debug.LogWarning("[SpiralSliceKaleidoscopeItemEffect] No profile.");
            yield break;
        }

        if (!profile.TryGet<SpiralSliceKaleidoscope>(out var fx) || fx == null)
        {
            if (verboseLogs) Debug.LogWarning("[SpiralSliceKaleidoscopeItemEffect] No SpiralSliceKaleidoscope.");
            yield break;
        }
        ...
        if (verboseLogs)
            Debug.Log("[SpiralSliceKaleidoscopeItemEffect] Start duration=" + duration + " target=" + target);
        ...
        fx.Fade.value = 0f;
        if (verboseLogs) Debug.Log("[SpiralSliceKaleidoscopeItemEffect] Done, reset.");
    ...
    private void ResetEffect()
    {
        ...
        if (profile != null && profile.TryGet<SpiralSliceKaleidoscope>(out var fx) && fx != null)
        {
            fx.Fade.overrideState = true;
            fx.Fade.value = 0f;
        }
