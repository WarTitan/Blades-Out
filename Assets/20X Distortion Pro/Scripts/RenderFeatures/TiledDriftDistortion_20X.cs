...
{
    public class TiledDriftDistortion_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    TiledDriftDistortion_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/TiledDriftDistortion");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new TiledDriftDistortion_Pass(material);
    ...
    {
        TiledDriftDistortion myVolume = VolumeManager.instance.stack?.GetComponent<TiledDriftDistortion>();
    ...
    public class TiledDriftDistortion_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int TileAmount = Shader.PropertyToID("TileAmount");
        static readonly int TileSize = Shader.PropertyToID("TileSize");
        static readonly int DriftSpeed = Shader.PropertyToID("DriftSpeed");
        private Material material;

        public TiledDriftDistortion_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<TiledDriftDistortion>();
            material.SetFloat("TileAmount", volumeComponent.Amount.value);
            material.SetFloat("TileSize", volumeComponent.Size.value);
            material.SetFloat("DriftSpeed", volumeComponent.Speed.value);
