...
{
    public class FracturedNoiseDisplacement_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    FracturedNoiseDisplacement_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/FracturedNoiseDisplacement");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new FracturedNoiseDisplacement_Pass(material);
    ...
    {
        FracturedNoiseDisplacement myVolume = VolumeManager.instance.stack?.GetComponent<FracturedNoiseDisplacement>();
    ...
    public class FracturedNoiseDisplacement_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int NoiseSpeed = Shader.PropertyToID("NoiseSpeed");
        static readonly int DisplaceMagnitude = Shader.PropertyToID("DisplaceMagnitude");
        static readonly int BlendFactor = Shader.PropertyToID("BlendFactor");
        static readonly int TempTargetId = Shader.PropertyToID("FracturedNoiseDisplacement");
        private Material material;

        public FracturedNoiseDisplacement_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<FracturedNoiseDisplacement>();

            material.SetFloat(BlendFactor, volumeComponent.Fade.value);
            material.SetFloat(DisplaceMagnitude, volumeComponent.Amount.value);
            material.SetFloat(NoiseSpeed, volumeComponent.Speed.value);
