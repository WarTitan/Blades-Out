...
{
    public class TwistShrinkDistortion_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    TwistShrinkDistortion_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/TwistShrinkDistortion");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new TwistShrinkDistortion_Pass(material);
    ...
    {
        TwistShrinkDistortion myVolume = VolumeManager.instance.stack?.GetComponent<TwistShrinkDistortion>();
    ...
    public class TwistShrinkDistortion_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int BlendFactor = Shader.PropertyToID("BlendFactor");
        static readonly int TwistStrength = Shader.PropertyToID("TwistStrength");
        static readonly int LayerPeriod = Shader.PropertyToID("LayerPeriod");
        static readonly int AnimSpeed = Shader.PropertyToID("AnimSpeed");
        private Material material;

        public TwistShrinkDistortion_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<TwistShrinkDistortion>();
            material.SetFloat(TwistStrength, volumeComponent.Intensity.value);
            material.SetFloat(LayerPeriod, volumeComponent.Delay.value);
