...
{
    public class VortexSwirlDisplacement_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    VortexSwirlDisplacement_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/VortexSwirlDisplacement");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new VortexSwirlDisplacement_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        VortexSwirlDisplacement myVolume = VolumeManager.instance.stack?.GetComponent<VortexSwirlDisplacement>();
    ...
    public class VortexSwirlDisplacement_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int AnimSpeed = Shader.PropertyToID("AnimSpeed");
        ...
        public VortexSwirlDisplacement_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<VortexSwirlDisplacement>();

            material.SetFloat(IterationCount, volumeComponent.Iterations.value);
            material.SetFloat(AnimSpeed, volumeComponent.Speed.value);
            material.SetFloat(BlendFactor, volumeComponent.Fade.value);
            material.SetVector("_CenterOffset", volumeComponent.CenterOffset.value);
            material.SetFloat("_RotScale", volumeComponent.RotationScale.value);
            material.SetFloat("_FalloffExp", volumeComponent.Falloff.value);
