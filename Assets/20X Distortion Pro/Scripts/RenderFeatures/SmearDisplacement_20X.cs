...
{
    public class SmearDisplacement_20X  : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    SmearDisplacement_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/SmearDisplacement");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new SmearDisplacement_Pass(material);
    ...
    {
        SmearDisplacement myVolume = VolumeManager.instance.stack?.GetComponent<SmearDisplacement>();
    ...
    public class SmearDisplacement_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int InvertEdge = Shader.PropertyToID("InvertEdge");
        static readonly int SmearStrength = Shader.PropertyToID("SmearStrength");
        private Material material;

        public SmearDisplacement_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<SmearDisplacement>();
            material.SetFloat(SmearStrength, volumeComponent.Intensity.value);
            material.SetFloat(InvertEdge, volumeComponent.Invert.value ? 0 : 1);
