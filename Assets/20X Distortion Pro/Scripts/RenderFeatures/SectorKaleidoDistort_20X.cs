...
{
    public class SectorKaleidoDistort_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    SectorKaleidoDistort_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/SectorKaleidoDistort");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new SectorKaleidoDistort_Pass(material);
    ...
    {
        SectorKaleidoDistort myVolume = VolumeManager.instance.stack?.GetComponent<SectorKaleidoDistort>();
    ...
    public class SectorKaleidoDistort_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int InnerRadius = Shader.PropertyToID("InnerRadius");
        static readonly int OuterRadius = Shader.PropertyToID("OuterRadius");
        static readonly int BlendFactor = Shader.PropertyToID("BlendFactor");
        static readonly int SectorIntensity = Shader.PropertyToID("SectorIntensity");
        static readonly int FillRatio = Shader.PropertyToID("FillRatio");
        static readonly int AddBlack = Shader.PropertyToID("AddBlack");
        private Material material;

        public SectorKaleidoDistort_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<SectorKaleidoDistort>();

            int shaderPass = 0;

            material.SetFloat(InnerRadius, volumeComponent.InnerRadius.value);
            material.SetFloat(OuterRadius, volumeComponent.OuterRadius.value);
            material.SetFloat(BlendFactor, volumeComponent.Fade.value);
            material.SetFloat(SectorIntensity, volumeComponent.Intensity.value);
            material.SetFloat(FillRatio, volumeComponent.Fill.value);
            material.SetInt(AddBlack, volumeComponent.Black.value ? 0 : 1);
