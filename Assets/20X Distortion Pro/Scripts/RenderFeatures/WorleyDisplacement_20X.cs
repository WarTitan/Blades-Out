...
{
    public class WorleyDisplacement_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    WorleyDisplacement_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/WorleyDisplacement");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new WorleyDisplacement_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        WorleyDisplacement myVolume = VolumeManager.instance.stack?.GetComponent<WorleyDisplacement>();
    ...
    public class WorleyDisplacement_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int DisplacePixels = Shader.PropertyToID("DisplacePixels");
        static readonly int BlendAmount = Shader.PropertyToID("BlendAmount");
        static readonly int NoiseScale = Shader.PropertyToID("NoiseScale");
        static readonly int FlowSpeed = Shader.PropertyToID("FlowSpeed");
        private Material material;

        public WorleyDisplacement_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<WorleyDisplacement>();

            int shaderPass = 0;

            material.SetFloat(DisplacePixels, volumeComponent.Amount.value);
            material.SetFloat(BlendAmount, volumeComponent.Fade.value);
            ...
            }else material.SetFloat(_FadeMultiplier, 0);
