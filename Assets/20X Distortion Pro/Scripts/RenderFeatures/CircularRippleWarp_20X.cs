...
{
    public class CircularRippleWarp_20X : ScriptableRendererFeature
    {
        [SerializeField]
        private Shader m_Shader;
        private Material material;
        CircularRippleWarp_Pass Distortion_RenderPass;
        public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

        public override void Create()
        {
            m_Shader = Shader.Find("DistortionsPro_20X/CircularRippleWarp");
            if (m_Shader == null)
            {
                return;
            }
            material = new Material(m_Shader);
            Distortion_RenderPass = new CircularRippleWarp_Pass(material);
            Distortion_RenderPass.renderPassEvent = Event;
        }

        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            CircularRippleWarp myVolume = VolumeManager.instance.stack?.GetComponent<CircularRippleWarp>();
        ...
        public class CircularRippleWarp_Pass : ScriptableRenderPass
        {
            static readonly int _Mask = Shader.PropertyToID("_Mask");
            static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
            static readonly int RippleFrequency = Shader.PropertyToID("RippleFrequency");
            static readonly int RippleDivider = Shader.PropertyToID("RippleDivider");
            static readonly int BlendFade = Shader.PropertyToID("BlendFade");
            static readonly int WaveSpeed = Shader.PropertyToID("WaveSpeed");
            private Material material;

            public CircularRippleWarp_Pass(Material material)
            {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                var volumeComponent = VolumeManager.instance.stack.GetComponent<CircularRippleWarp>();
                int shaderPass = 0;

                material.SetFloat(RippleDivider, volumeComponent.Zoom.value);
                material.SetFloat(RippleFrequency, volumeComponent.RingsCount.value);
                material.SetFloat(BlendFade, volumeComponent.Fade.value);
                material.SetFloat(WaveSpeed, volumeComponent.Speed.value);
