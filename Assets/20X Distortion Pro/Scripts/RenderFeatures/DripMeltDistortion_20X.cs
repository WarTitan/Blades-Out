...
{
    public class DripMeltDistortion_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    DripMeltDistortion_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/DripMeltDistortion");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new DripMeltDistortion_Pass(material);
    ...
    {
        DripMeltDistortion myVolume = VolumeManager.instance.stack?.GetComponent<DripMeltDistortion>();
    ...
    public class DripMeltDistortion_Pass : ScriptableRenderPass
    {
        ...
        static readonly int WaveScale = Shader.PropertyToID("WaveScale");
        private Material material;

        public DripMeltDistortion_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<DripMeltDistortion>();

            material.SetFloat(WaveSpeed, volumeComponent.Speed.value);
            material.SetFloat(DistortStrength, volumeComponent.Strength.value);
            material.SetFloat(WaveScale, volumeComponent.Size.value);
