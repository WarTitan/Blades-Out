...
{
    public class FluidNoiseDistortion_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    FluidNoiseDistortion_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/FluidNoiseDistortion");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new FluidNoiseDistortion_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        FluidNoiseDistortion myVolume = VolumeManager.instance.stack?.GetComponent<FluidNoiseDistortion>();
    ...
    public class FluidNoiseDistortion_Pass : ScriptableRenderPass
    {
        ...
        static readonly int CustomTime = Shader.PropertyToID("CustomTime");
        private Material material;

        public FluidNoiseDistortion_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<FluidNoiseDistortion>();
            material.SetFloat(DistortStrength, volumeComponent.Intensity.value);
            TimeX += Time.deltaTime * volumeComponent.Speed.value;
            material.SetFloat(CustomTime, TimeX);
