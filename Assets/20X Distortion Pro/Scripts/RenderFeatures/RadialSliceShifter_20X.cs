...
{
    public class RadialSliceShifter_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    RadialSliceShifter_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/RadialSliceShifter");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new RadialSliceShifter_Pass(material);
    ...
    {
        RadialSliceShifter myVolume = VolumeManager.instance.stack?.GetComponent<RadialSliceShifter>();
    ...
    public class RadialSliceShifter_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int BaseSliceWidth = Shader.PropertyToID("BaseSliceWidth");
        static readonly int LineExpansionRate = Shader.PropertyToID("LineExpansionRate");
        static readonly int Phase = Shader.PropertyToID("Phase");
        private Material material;

        public RadialSliceShifter_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<RadialSliceShifter>();
            TimeX += Time.deltaTime * volumeComponent.Speed.value;
            material.SetFloat(Phase, TimeX);
            material.SetFloat(BaseSliceWidth, volumeComponent.LineWidth.value);
            material.SetFloat(LineExpansionRate, volumeComponent.AnimateLine.value ? 1 : 0);
