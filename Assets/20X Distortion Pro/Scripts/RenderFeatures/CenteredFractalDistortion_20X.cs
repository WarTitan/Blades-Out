...
{
    public class CenteredFractalDistortion_20X : ScriptableRendererFeature
    {
        [SerializeField]
        private Shader m_Shader;
        private Material material;
        CenteredFractalDistortion_Pass Distortion_RenderPass;
        public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

        public override void Create()
        {
            m_Shader = Shader.Find("DistortionsPro_20X/CenteredFractalDistortion");
            if (m_Shader == null)
            {
                return;
            }
            material = new Material(m_Shader);
            Distortion_RenderPass = new CenteredFractalDistortion_Pass(material);
        ...
        {
            CenteredFractalDistortion myVolume = VolumeManager.instance.stack?.GetComponent<CenteredFractalDistortion>();
        ...
        public class CenteredFractalDistortion_Pass : ScriptableRenderPass
        {
            static readonly int _Mask = Shader.PropertyToID("_Mask");
            static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
            static readonly int MaskThreshold = Shader.PropertyToID("MaskThreshold");
            static readonly int NoiseScale = Shader.PropertyToID("NoiseScale");
            static readonly int TimeFactor = Shader.PropertyToID("TimeFactor");
            static readonly int DistortStrength = Shader.PropertyToID("DistortStrength");

            private Material material;

            public CenteredFractalDistortion_Pass(Material material)
            {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                var volumeComponent = VolumeManager.instance.stack.GetComponent<CenteredFractalDistortion>();

                int shaderPass = 0;
                if (volumeComponent.CubicNoise.value)
                    shaderPass = 0;
                else
                    shaderPass = 1;

                TimeX += Time.deltaTime * volumeComponent.Speed.value;
                ParamSwitch(material, volumeComponent.CenterFocus.value, "FOCUS_CENTER");
                material.SetFloat(NoiseScale, volumeComponent.Size.value);
                ...
                material.SetFloat(TimeFactor, TimeX);
                material.SetFloat(DistortStrength, volumeComponent.Intensity.value);
