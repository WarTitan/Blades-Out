...
{
    public class SpeedWarp_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    SpeedWarp_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/SpeedWarp");
        if (m_Shader == null) return;
        material = new Material(m_Shader);
        Distortion_RenderPass = new SpeedWarp_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        SpeedWarp myVolume = VolumeManager.instance.stack?.GetComponent<SpeedWarp>();
    ...
    public class SpeedWarp_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int NoiseFrequency = Shader.PropertyToID("NoiseFrequency");
        static readonly int WarpStrength = Shader.PropertyToID("WarpStrength");
        static readonly int FlowRate = Shader.PropertyToID("FlowRate");
        static readonly int WarpCenter = Shader.PropertyToID("WarpCenter");
        private Material material;

        public SpeedWarp_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<SpeedWarp>();
            TimeX += Time.deltaTime;


            material.SetFloat(FlowRate, volumeComponent.Speed.value);
            material.SetFloat(NoiseFrequency, volumeComponent.Frequency.value);
            material.SetFloat(WarpStrength, volumeComponent.Intensity.value);
            ...
            material.SetVector(WarpCenter, volumeComponent.Center.value);
            material.SetFloat("T", TimeX);
