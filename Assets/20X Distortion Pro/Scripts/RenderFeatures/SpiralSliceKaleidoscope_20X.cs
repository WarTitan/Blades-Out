...
{
    public class SpiralSliceKaleidoscope_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    SpiralSliceKaleidoscope_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/SpiralSliceKaleidoscope");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new SpiralSliceKaleidoscope_Pass(material);
    ...
    {
        SpiralSliceKaleidoscope myVolume = VolumeManager.instance.stack?.GetComponent<SpiralSliceKaleidoscope>();
    ...
    public class SpiralSliceKaleidoscope_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int IterationCount = Shader.PropertyToID("IterationCount");
        static readonly int SpiralScale = Shader.PropertyToID("SpiralScale");
        static readonly int SpiralRotation = Shader.PropertyToID("SpiralRotation");
        static readonly int SpiralOffset = Shader.PropertyToID("SpiralOffset");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int NoiseWarpStrength = Shader.PropertyToID("NoiseWarpStrength");
        static readonly int BlendFactor = Shader.PropertyToID("BlendFactor");
        static readonly int AnimationSpeed = Shader.PropertyToID("AnimationSpeed");
        static readonly int BaseDarkening = Shader.PropertyToID("BaseDarkening");
        private Material material;

        public SpiralSliceKaleidoscope_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<SpiralSliceKaleidoscope>();
            material.SetInt(IterationCount, volumeComponent.Images.value);
            material.SetFloat(SpiralScale, volumeComponent.Scale.value);
            material.SetFloat(SpiralRotation, volumeComponent.Rotation.value);
            material.SetFloat(SpiralOffset, volumeComponent.Offset.value);
            material.SetFloat(NoiseWarpStrength, volumeComponent.NoiseWarpStrength.value);
            material.SetFloat(BlendFactor, volumeComponent.Fade.value);
            material.SetFloat(AnimationSpeed, volumeComponent.Speed.value);
            material.SetFloat(BaseDarkening, volumeComponent.Black.value?0:1);
