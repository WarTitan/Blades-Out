...
{
    public class VoronoiPatterns_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    VoronoiPatterns_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/VoronoiPatterns");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new VoronoiPatterns_Pass(material);
    ...
    {
        VoronoiPatterns myVolume = VolumeManager.instance.stack?.GetComponent<VoronoiPatterns>();
    ...
    public class VoronoiPatterns_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int CellScale = Shader.PropertyToID("CellSize");
        static readonly int FlowSpeed = Shader.PropertyToID("FlowRate");
        static readonly int BlendStrength = Shader.PropertyToID("BlendFactor");
        static readonly int PatternVariant = Shader.PropertyToID("DisplayMode");
        static readonly int RidgeThickness = Shader.PropertyToID("Thickness");

        private Material material;

        public VoronoiPatterns_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<VoronoiPatterns>();
            int shaderPass = 0;

            material.SetFloat(CellScale, volumeComponent.Size.value);
            material.SetFloat(RidgeThickness, volumeComponent.Thikness.value);
            material.SetFloat(FlowSpeed, volumeComponent.Speed.value);
            material.SetFloat(BlendStrength, volumeComponent.Fade.value);
            material.SetInt(PatternVariant, (int)volumeComponent.DisplaceMode.value);
