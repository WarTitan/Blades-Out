...
{
    public class VortexPixelShatter_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    VortexPixelShatter_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/VortexPixelShatter");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new VortexPixelShatter_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        VortexPixelShatter myVolume = VolumeManager.instance.stack?.GetComponent<VortexPixelShatter>();
    ...
    public class VortexPixelShatter_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int PixelateStrength = Shader.PropertyToID("PixelateStrength");
        static readonly int ShatterCenter = Shader.PropertyToID("ShatterCenter");
        private Material material;

        public VortexPixelShatter_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<VortexPixelShatter>();
            material.SetFloat(PixelateStrength, volumeComponent.Amount.value);
            material.SetVector(ShatterCenter, volumeComponent.Center.value);
