...
{
    public class FluidWorleyDisplacement_20X : ScriptableRendererFeature
{
    [SerializeField]
    private Shader m_Shader;
    private Material material;
    FluidWorleyDisplacement_Pass Distortion_RenderPass;
    public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("DistortionsPro_20X/FluidWorleyDisplacement");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Distortion_RenderPass = new FluidWorleyDisplacement_Pass(material);
        Distortion_RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        FluidWorleyDisplacement myVolume = VolumeManager.instance.stack?.GetComponent<FluidWorleyDisplacement>();
    ...
    public class FluidWorleyDisplacement_Pass : ScriptableRenderPass
    {
        static readonly int _Mask = Shader.PropertyToID("_Mask");
        static readonly int _FadeMultiplier = Shader.PropertyToID("_FadeMultiplier");
        static readonly int DisplaceAmount = Shader.PropertyToID("DisplaceAmount");
        static readonly int FlowVelocity = Shader.PropertyToID("FlowVelocity");
        static readonly int NoiseScale = Shader.PropertyToID("NoiseScale");
        static readonly int BlendIntensity = Shader.PropertyToID("BlendIntensity");
        private Material material;

        public FluidWorleyDisplacement_Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<FluidWorleyDisplacement>();
            material.SetFloat(BlendIntensity, volumeComponent.Intensity.value);

            material.SetFloat(DisplaceAmount, volumeComponent.Offset.value);
            material.SetFloat(FlowVelocity, volumeComponent.Speed.value * 10);
